...
export default function EditProfile() {
  ...
  useEffect(() => {
    ...
    getDocs(newCollectionRef).then((querySnapshot) => {
      querySnapshot.forEach((doc) => {
        ...
        setOldUserInfo(doc.data());
        setUserName(doc.data().userName);
        setUserSurname(doc.data().userSurname);
        setUserAge(doc.data().age);
        setUserHeight(doc.data().height);
  ...
  return (
    <View style={tw`h-full`}>
      ...
      <View style={tw`w-full h-100 mx-auto mt-5`}>
        <Text style={tw`text-3xl font-bold text-indigo-700 text-center`}>Edit Your Profile</Text>
        <View style={tw`mx-auto w-content`}>
          <TextInput
            label={"Name"}
            mode='outlined'
            style={tw`w-80 h-15`}
        ...
        <View style={tw`mx-auto mt-3`}>
          ...
          <TextInput
            ...
            mode='outlined'
            value={userSurname}
            onChangeText={surname => setUserSurname(surname)}
            style={tw`w-80 h-15`}
          />
