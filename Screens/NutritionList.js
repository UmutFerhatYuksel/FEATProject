...
export default function MealPlan({ navigation, route }) {
    ...
    return (
        <ScrollView style={{ padding: 24, flex: 1, backgroundColor: 'white' }}>
            ...
            <Text style={{ fontSize: 24, textAlign: 'center', fontWeight: 'bold', marginTop: 12 }}>Your Daily Nutrition List</Text>
            {todaysNutrition.map((item) => (

                <>
                    <View style={{ marginTop: 36 }}>
                        <View style={{ backgroundColor: '#F6F7F7', padding: 10, borderRadius: 6, marginTop: 14 }}>
                            <Pressable onPress={()=>navigation.navigate("NutritionDetail",{item:item})} style={{ backgroundColor: '#F6F7F7', padding: 10, borderRadius: 4, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                                <View>
