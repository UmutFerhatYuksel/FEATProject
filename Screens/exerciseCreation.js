...
const createExercise = ({difficulty, exerciseDay, equipment}) => {
/*
Adım 1: Level'a göre bütün egzersizler filtrelenir
Adım 2: Antrenman gününe göre (örnek push) kas grupları çıkarılır
    Push: Chest, triceps, shoulder (mid and front)
    Pull: Back, biceps, rear shoulder
    Legs: Quads, hamstrings, glutes, calves
Adım 3: Kas grupları daha detaylı filtrelenir 
    örnek: triceps lateral head, short head, medial head 
Adım 4: Uygun ekipman seçilir
Adım 5: Her detaylı kas grubu için 3 setlik hareket seçilir    
*/
...
const filterExercises = (difficulty, exerciseDay, equipment) => {
    exerciseList.filter(exercise => exercise.difficulty === difficulty);
    ...
    exerciseList.filter(exercise => muscleGroups.every(m => exercise.muscle_target.includes(m)));
    exerciseList.filter(exercise => exercise.equipment === equipment);
}

const generateWorkout= (exercises) => {
    const workoutSetsReps = [];
    exercises.forEach(exercise => {
        // Generate 3 sets of 8 reps for each exercise
        for (let i = 0; i < 3; i++) {
            workoutSetsReps.push({
    ...
    return workoutSetsReps;
};


filterExercises(difficulty, exerciseDay, equipment);
