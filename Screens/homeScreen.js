...
const personalInfoScreen = ({ navigation, route }) => {
  ...
  return (
    <View style={tw`flex-1 justify-center items-center bg-teal-600`}>
      <Text style={tw`text-white text-3xl font-bold mt-30`}>Choose Proper Exercises</Text>
      <View style={tw`h-fit w-fit border-black border-2 rounded-2xl bg-white mx-auto`}>
        <ScrollView>
          {Exercises.map((item, index) => {
            if (exerciseLevel === item.difficulty) {
              return (<View style={tw`flex flex-row`}>
                <View style={tw`p-2`}><Text>Name: {item.name}</Text>  </View>
                <View style={tw`p-2`}><Text>Muscle: {item.muscle}</Text></View>
                <View style={tw`p-2`}><Text>Type: {item.type}</Text></View>
                <View style={tw`mr-5`}>
                  <BouncyCheckbox
                    size={25}
                    fillColor="red"
                    ...
                    onPress={(isChecked) => { handleCheckboxToggle(isChecked, item) }}
                  />
                </View>
              </View>
              )
